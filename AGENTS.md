...
## Keywords
ocserv, openconnect, anyconnect, mcp, vpn-admin, access-control, policy-groups, infrastructure-automation, audit-logging, agent-integration

## Annotation
A generic safe administrative package for `ocserv` that lets an operator manage VPN users, policy groups, and active sessions without manually editing server configuration. It exposes a constrained MCP tool surface and localhost backend for safe execution on the VPS, and can be integrated into Nanobot or any other MCP-capable agent.
